...
key-generate: ## Sets the src/Application key
...
phpmd: ## Runs php mess detector
	@make exec cmd="php ./vendor/bin/phpmd src/App,tests text phpmd_ruleset.xml --suffixes php"
...
ifeq ($(INSIDE_DOCKER_CONTAINER), 1)
	...
	@./vendor/bin/phpstan analyze src/App tests
else
