...
locals {
  resources_tags = {
    Name          = "${var.rds_name}",
    ResourceGroup = "${var.environment[local.env]}-RDS-tensorwarp"
  }
}
...
## Create New KMS
# resource "aws_kms_key" "tensorwarp_cmk_sym" {
...
resource "aws_security_group" "rds_default" {
  name        = "tensorwarp-sg-rds-laraveldb-${var.env[local.env]}"
...
module "db" {
  ...
  ## Create New KMS
  # kms_key_id      = aws_kms_key.tensorwarp_cmk_sym.arn
  ## Existing CMK ARN
