...
locals {
  resources_tags = {
    ...
    ResourceGroup = "${var.environment[local.env]}-EKS-tensorwarp"
  }
}
...
resource "aws_eks_cluster" "aws_eks" {
  ...
  tags = merge(
    ...
    },
    {
      ...
      ProductGroup    = "${var.env[local.env]}" == "prod" ? "PROD-EKS-tensorwarp" : "STG-EKS-tensorwarp"
      Department      = "DEVOPS"
      DepartmentGroup = "${var.env[local.env]}" == "prod" ? "PROD-DEVOPS" : "STG-DEVOPS"
      ResourceGroup   = "${var.env[local.env]}" == "prod" ? "PROD-EKS-tensorwarp" : "STG-EKS-tensorwarp"
      Services        = "EKS"
    }
  )
